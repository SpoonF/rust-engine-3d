use sdl2::{Sdl,event::Event, keyboard::Keycode, pixels::{Color, PixelFormat, PixelFormatEnum}, rect::Rect, render::{Canvas, Texture, TextureCreator}, video::{Window, WindowContext} };

use crate::{geometry::{Vector2D, Vector3D}};
pub struct Scene {
    scene: Vec<Vec<u32>>,
    zbuffer: Vec<i32>,
...
impl Scene {
    ...
    //         std::mem::swap(&mut p[0].x, &mut p[0].y);
    //         std::mem::swap(&mut p[1].x, &mut p[1].y);
    //         steep = true;
    //     }

    //     if p[0].x > p[1].x {
    //         std::mem::swap(&mut p[0].x, &mut p[1].x);
    //         std::mem::swap(&mut p[0].y, &mut p[1].y);
    //     }
    //     let dx = p[1].x-p[0].x;
    //     let dy = p[1].y-p[0].y;
    ...
    pub fn triangle_3d(&mut self, mut p: [Vector3D<i32>; 3], color: u32) {
        ...
        for i in 0..total_height {
            ...
            for j in a.x as i32..=b.x as i32 {
                ...
                if self.zbuffer[idx] < pp.z {
                    ...
                    self.scene[pp.x as usize][pp.y as usize] = color;
                }
            }
        }
    }
