...
use std::mem;
use crate::{geometry::{Vector2D, Vector3D}, model::Model, tga::Tga};
pub struct Scene {
    scene: Vec<Vec<u32>>,
    zbuffer: Vec<i32>,
...
impl Scene {
    ...
    pub fn triangle_3d(&mut self, mut p: [Vector3D<i32>; 3], mut uv: [Vector2D<i32>; 3], intesity: f32, model: &Model) {
        if p[0].y == p[1].y && p[0].y == p[2].y {
            return
        }; 
        if p[0].y>p[1].y { p.swap(0, 1); uv.swap(0, 1);};
        if p[0].y>p[2].y { p.swap(0, 2); uv.swap(0, 2);};
        if p[1].y>p[2].y { p.swap(1, 2); uv.swap(1, 2);};
        ...
        for i in 0..total_height {
            ...
            for j in a.x as i32..=b.x as i32 {
                ...
                if self.zbuffer[idx] < pp.z {
                    ...
                    self.set(pp.x as usize, pp.y as usize, get_color(color, intesity));
                }
            }
        }
    }
    pub fn triangle_new(&mut self, mut p: [Vector3D<i32>; 3], mut ity: [f32; 3]) {
        if p[0].y == p[1].y && p[0].y == p[2].y {
            return
        }; 
        if p[0].y>p[1].y { p.swap(0, 1); ity.swap(0, 1);};
        if p[0].y>p[2].y { p.swap(0, 2); ity.swap(0, 2);};
        if p[1].y>p[2].y { p.swap(1, 2); ity.swap(1, 2);};
        ...
        for i in 0..total_height {
            ...
            for j in a.x as i32..=b.x as i32 {
                ...
                if self.zbuffer[idx] < p.z {
                    ...
                    self.set(p.x as usize, p.y as usize, get_color(255, ityp));
                }
            }
        }
    }
